...
import { useLedgerPrivacy } from '@/components/LedgerPrivacyProvider';

const options = [
  { value: 0, emoji: '👀', label: '顯示全部', description: '顯示所有金額' },
  { value: 1, emoji: '🫣', label: '隱藏明細', description: '隱藏每一筆的交易金額與分攤' },
  { value: 2, emoji: '😎', label: '隱藏總收支', description: '隱藏上方總收入與總支出' },
  { value: 3, emoji: '🙈', label: '隱藏所有金額', description: '隱藏所有明細與總收支金額' },
];

export function LedgerPrivacyDropdown({ variant = 'icon' }: { variant?: 'icon' | 'full' }) {
  const { ledgerPrivacyLevel, setLedgerPrivacyLevel } = useLedgerPrivacy();
  ...
  const currentOption = options.find((opt) => opt.value === ledgerPrivacyLevel) || options[0];
            ...
            {options.map((option) => (
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className={`text-sm font-medium ${ledgerPrivacyLevel === option.value ? 'text-blue-700 dark:text-blue-400' : 'text-zinc-900 dark:text-zinc-100'}`}>
                    {option.label}
                  </span>
                  <span className={`text-xs mt-0.5 ${ledgerPrivacyLevel === option.value ? 'text-blue-600/80 dark:text-blue-400/80' : 'text-zinc-500 dark:text-zinc-400'}`}>
                    {option.description}
                  </span>
                </div>
              </button>
