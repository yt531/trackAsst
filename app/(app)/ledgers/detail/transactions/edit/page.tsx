...
import { Category, LedgerMember, TransactionSplit, Tag, Transaction } from '@/types';
...
function SharedTransactionForm() {
  ...
  const txId = searchParams.get('txId') as string;
    ...
    try {
      ...
      if (txDoc.exists()) {
        ...
        if (activeLedger?.mode === 'split' && tx.splits) {
          const payer = tx.splits.find(s => s.paidAmount > 0);
          setPayerId(payer ? payer.userId : user.uid);
          const splitters = tx.splits.filter(s => s.owedAmount > 0).map(s => s.userId);
          setSplitWithIds(splitters);
        } else {
          setPayerId(tx.userId || user.uid);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateTransaction(user.uid, txId, {
        userId: payerId, // Setting user to payer
        type,
        amount: numAmount,
        baseAmount,
        categoryId,
      ...
      router.push(`/ledgers/detail/transactions?id=${activeLedgerId}`);
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6 pb-20">
      <header>
        <h1 className="text-2xl font-bold tracking-tight">編輯共享交易</h1>
