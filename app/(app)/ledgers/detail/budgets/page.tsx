...
import { getLedgerBudgetsByMonth, saveLedgerBudget, deleteLedgerBudget, updateLedgerBudgetOrders } from '@/lib/ledgerBudget';
...
function SortableBudgetCard({ budget, categoryName, selectedMonth, transactions, onEdit, onDelete, onConfigRules, ledgerId, isReorderMode }: SortableBudgetCardProps) {
  ...
  const isCurrentYear = selectedMonth.substring(0, 4) === format(new Date(), 'yyyy');
  ...
  if (!budget.categoryId) {
    ...
    relevantTxs.forEach(t => {
      ...
      if (rule === 'deduction') {
        rawSpent += t.baseAmount || t.amount;
      } else if (rule === 'addition') {
        additions += t.baseAmount || t.amount;
      }
    });
    ...
    if (budget.period === 'monthly' || budget.period === 'yearly' || budget.period === 'weekly') {
      ...
      if (isCurrentMonth) {
        ...
        spent = todayTxs.reduce((sum, t) => sum + (t.baseAmount || t.amount), 0);
      } else {
        const daysInMonth = new Date(Number(selectedMonth.split('-')[0]), Number(selectedMonth.split('-')[1]), 0).getDate();
        const totalSpent = categoryTxs.reduce((sum, t) => sum + (t.baseAmount || t.amount), 0);
...
export default function LedgerBudgetsPage() {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (settings?.ledgerVisibleBudgetPeriods?.[ledgerId] && settings.ledgerVisibleBudgetPeriods[ledgerId].length > 0) {
          const p = settings.ledgerVisibleBudgetPeriods[ledgerId];
          setVisiblePeriods(p);
          setTempVisiblePeriods(p);
          setViewMode(prev => p.includes(prev) ? prev : p[0]);
        }
      } catch (e) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !ledgerId) return;
    ...
    setBudgets((items) => {
      ...
      updateLedgerBudgetOrders(ledgerId, updates).catch(e => console.error("Error updating budget orders", e));
      return updatedItems;
    });
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setBudgets(fetchedBudgets.filter(b => b.period === viewMode));

        const catRef = collection(db, 'ledgers', ledgerId, 'categories');
  ...
  const handleSaveConfig = async () => {
    if (!user || !configBudgetId || !ledgerId) return;
    try {
      const existing = budgets.find(b => b.id === configBudgetId);
      if (!existing) return;
      
      const updatedBudget = await saveLedgerBudget(ledgerId, {
  ...
  const handleSaveBudget = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !amount || !ledgerId) return;
    ...
    try {
      ...
      if (editingBudgetId && editingBudgetId !== newBudget.id) {
        await deleteLedgerBudget(ledgerId, editingBudgetId);
      }
  ...
  const handleDeleteBudget = async (budgetId: string) => {
    if (!ledgerId || !confirm('確定要刪除這個預算嗎？')) return;
    try {
      await deleteLedgerBudget(ledgerId, budgetId);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="flex flex-1 bg-zinc-100 dark:bg-zinc-800/50 p-1 rounded-xl">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 w-full mb-6">
        <button
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl dark:bg-zinc-800">
            ...
            <p className="mb-4 text-sm text-zinc-500 dark:text-zinc-400">此設定僅套用於目前的共用帳本。資料不會被刪除，重新勾選即可恢復顯示。</p>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-700">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const settings = await getUserSettings(user.uid) || {};
                    const newLedgerSettings = {
                      ...(settings.ledgerVisibleBudgetPeriods || {}),
                      [ledgerId]: tempVisiblePeriods
                    };
                    await setUserSettings(user.uid, { ledgerVisibleBudgetPeriods: newLedgerSettings });
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-12 text-center dark:border-zinc-700 dark:bg-zinc-800/50">
